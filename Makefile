...
STAGE3_BIN = make/boot/x64/stage3.bin
...
POLICY_SECTOR = 4096
# Phase 67: FAT32 test volume (36MB, built with mtools) at sector 8192
FAT32_SECTOR = 8192
FAT32_IMG    = make/fat32.img
FAT32_MB     = 36
# Phase 68: ext2 test volume (4MB, built with ai/tools/mkfs_ext2.py) at sector 81920
EXT2_SECTOR = 81920
EXT2_IMG    = make/ext2.img
EXT2_MB     = 4
...
# Phase 65: tcpsrv (SYS_LISTEN/SYS_ACCEPT demo)
USER_TS_SRC = $(USER_DIR)/tcpsrv.c
USER_TS_X86_O = $(USER_OUT_DIR)/tcpsrv_x86.o
USER_TS_X64_O = $(USER_OUT_DIR)/tcpsrv_x64.o
USER_TS_X86_ELF = $(USER_OUT_DIR)/tcpsrv32.elf
USER_TS_X64_ELF = $(USER_OUT_DIR)/tcpsrv64.elf

# Phase 69: forktest (CoW fork verification)
USER_FT_SRC = $(USER_DIR)/forktest.c
USER_FT_X86_O = $(USER_OUT_DIR)/forktest_x86.o
USER_FT_X64_O = $(USER_OUT_DIR)/forktest_x64.o
USER_FT_X86_ELF = $(USER_OUT_DIR)/forktest32.elf
USER_FT_X64_ELF = $(USER_OUT_DIR)/forktest64.elf
...
$(UNIVERSAL_IMG): $(STAGE1_BIN) $(STAGE2_BIN) $(STAGE3_BIN) \
                  $(KERNEL_X86_BIN) $(KERNEL_X64_BIN) $(POLICY_BIN) $(VFS_BIN) $(FAT32_IMG) $(EXT2_IMG)

	@echo "Creating universal image..."
	rm -f $@
	dd if=/dev/zero of=$@ bs=1M count=44 status=none
	...
	@echo "Adding FAT32 volume at sector $(FAT32_SECTOR)..."
	dd if=$(FAT32_IMG) of=$@ bs=512 seek=$(FAT32_SECTOR) conv=notrunc

	@echo "Adding ext2 volume at sector $(EXT2_SECTOR)..."
	dd if=$(EXT2_IMG) of=$@ bs=512 seek=$(EXT2_SECTOR) conv=notrunc

# Phase 68: ext2 test volume built with the pure-Python formatter.
$(EXT2_IMG): ai/tools/mkfs_ext2.py | prepare
	echo "Hello from ext2 on VernisOS!" > make/ext2_hello.txt
	echo "ext2 nested file works" > make/ext2_nested.txt
	python3 ai/tools/mkfs_ext2.py -o $@ --size-mb $(EXT2_MB) \
	    --add make/ext2_hello.txt=/hello.txt \
	    --add make/ext2_nested.txt=/subdir/nested.txt

# Phase 67: FAT32 test volume built with mtools (mformat/mcopy/mmd).
# Contains fixtures the fatls/fatcat CLI tests read back.
$(FAT32_IMG): | prepare
	dd if=/dev/zero of=$@ bs=1M count=$(FAT32_MB) status=none
	mformat -i $@ -F -v VERNIS ::
	echo "Hello from FAT32 on VernisOS!" > make/fat_hello.txt
	echo "nested file works" > make/fat_nested.txt
	mcopy -i $@ make/fat_hello.txt ::/HELLO.TXT
	mmd -i $@ ::/SUBDIR
	mcopy -i $@ make/fat_nested.txt ::/SUBDIR/NESTED.TXT
...
# ==== VernisFS image ====
$(VFS_BIN): ai/tools/mkfs_vernis.py $(USER_VSH_X86_ELF) $(USER_VSH_X64_ELF) $(USER_GETTY_X86_ELF) $(USER_GETTY_X64_ELF) $(USER_LOGIN_X86_ELF) $(USER_LOGIN_X64_ELF) $(USER_INIT_X86_ELF) $(USER_INIT_X64_ELF) $(USER_NC_X86_ELF) $(USER_NC_X64_ELF) $(USER_TS_X86_ELF) $(USER_TS_X64_ELF) $(USER_FT_X86_ELF) $(USER_FT_X64_ELF) | prepare
	@echo "Creating VernisFS image..."
	python3 ai/tools/mkfs_vernis.py -o $(VFS_BIN) --vsh32 $(USER_VSH_X86_ELF) --vsh64 $(USER_VSH_X64_ELF) --getty32 $(USER_GETTY_X86_ELF) --getty64 $(USER_GETTY_X64_ELF) --login32 $(USER_LOGIN_X86_ELF) --login64 $(USER_LOGIN_X64_ELF) --init32 $(USER_INIT_X86_ELF) --init64 $(USER_INIT_X64_ELF) --nc32 $(USER_NC_X86_ELF) --nc64 $(USER_NC_X64_ELF) --tcpsrv32 $(USER_TS_X86_ELF) --tcpsrv64 $(USER_TS_X64_ELF) --forktest32 $(USER_FT_X86_ELF) --forktest64 $(USER_FT_X64_ELF)
...
# ==== Phase 65: tcpsrv (listen/accept demo) ====
$(USER_TS_X86_O): $(USER_TS_SRC) $(USER_SYSCALL_HDR) $(USER_LIBC_HDR) | prepare
	$(CC_X86) $(USER_CFLAGS_X86) -c $< -o $@

$(USER_TS_X64_O): $(USER_TS_SRC) $(USER_SYSCALL_HDR) $(USER_LIBC_HDR) | prepare
	$(CC_X64) $(USER_CFLAGS_X64) -c $< -o $@

$(USER_TS_X86_ELF): $(USER_CRT0_X86_O) $(USER_LIBC_X86_O) $(USER_TS_X86_O) $(USER_LD_X86) | prepare
	$(LD_X86) -T $(USER_LD_X86) -m elf_i386 --gc-sections $(USER_CRT0_X86_O) $(USER_LIBC_X86_O) $(USER_TS_X86_O) -o $@

$(USER_TS_X64_ELF): $(USER_CRT0_X64_O) $(USER_LIBC_X64_O) $(USER_TS_X64_O) $(USER_LD_X64) | prepare
	$(LD_X64) -T $(USER_LD_X64) -m elf_x86_64 --gc-sections $(USER_CRT0_X64_O) $(USER_LIBC_X64_O) $(USER_TS_X64_O) -o $@

# ==== Phase 69: forktest (CoW verification) ====
$(USER_FT_X86_O): $(USER_FT_SRC) $(USER_SYSCALL_HDR) $(USER_LIBC_HDR) | prepare
	$(CC_X86) $(USER_CFLAGS_X86) -c $< -o $@

$(USER_FT_X64_O): $(USER_FT_SRC) $(USER_SYSCALL_HDR) $(USER_LIBC_HDR) | prepare
	$(CC_X64) $(USER_CFLAGS_X64) -c $< -o $@

$(USER_FT_X86_ELF): $(USER_CRT0_X86_O) $(USER_LIBC_X86_O) $(USER_FT_X86_O) $(USER_LD_X86) | prepare
	$(LD_X86) -T $(USER_LD_X86) -m elf_i386 --gc-sections $(USER_CRT0_X86_O) $(USER_LIBC_X86_O) $(USER_FT_X86_O) -o $@

$(USER_FT_X64_ELF): $(USER_CRT0_X64_O) $(USER_LIBC_X64_O) $(USER_FT_X64_O) $(USER_LD_X64) | prepare
	$(LD_X64) -T $(USER_LD_X64) -m elf_x86_64 --gc-sections $(USER_CRT0_X64_O) $(USER_LIBC_X64_O) $(USER_FT_X64_O) -o $@
...
prerequisites:
	...
	@printf "  %-25s " "mtools (mformat)" && (which mformat > /dev/null 2>&1 && echo "OK" || echo "MISSING (needed for FAT32 test volume)")
