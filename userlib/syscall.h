...
/* Phase 65: TCP server. bind_port() records the port, listen() opens the
   passive socket, accept() blocks (bounded, ~30s) for a connection and
   returns a new fd usable with read()/write()/close(). */
static inline int listen(int fd) {
    return (int)_syscall3(SYS_LISTEN, (size_t)fd, 0, 0);
}

static inline int accept(int fd) {
    return (int)_syscall3(SYS_ACCEPT, (size_t)fd, 0, 0);
}
