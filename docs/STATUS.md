...
> อัปเดตล่าสุด: 2026-08-19 | Version: 0.17.0
> Phase 72 (2026-08-19): TCP out-of-order reassembly — single-slot buffer เก็บ
> segment ที่มาก่อน gap แล้ว flush ต่อเมื่อ gap-filler มาถึง (เดิม dup-ACK
> แล้ว drop ทิ้ง) — host-side test ใหม่ที่ kernel/net/tests/test_tcp_ooo.c
> Phase 65-69 (2026-07-04): userland TCP server (SYS_LISTEN/ACCEPT), TCP sliding
> window + retransmit, FAT32 read driver, ext2 read driver, CoW fork
> (+ แก้ SYS_EXIT terminate ผิด pid)
> Phase 70 (2026-07-04): FAT32/ext2 mount เข้า kfs_* VFS tree ที่ /mnt/fat32,
> /mnt/ext2 (ls/cat ใช้ได้ตรง ๆ); แก้ integration suite 2 test ที่พังจริง
> (ipc_recv ใช้ pid แทน qid, check_access scale ผิด) + su ไม่รองรับ root
> passwordless + test harness desync (CRLF, ps realtime, prompt match) —
> ทั้ง 2 arch เขียว 31/31 ตอนนี้
> Phase 71 (2026-07-04): waitpid(pid) block จริงในเคอร์เนล (bounded ~30s,
> sti/pause/cli idiom เดียวกับ accept/connect) แทนที่จะ return -1 ทันที
> แล้วให้ userland spin-yield เอง — ใช้ scheduler_get_process_info() แยก
> "ยังรันอยู่" ออกจาก "exit code -1"/"pid ไม่รู้จัก" (get_exit_code เดิม
> คืน -1 เหมือนกันหมดทั้ง 3 กรณี, แยกไม่ออก)
...
| 16 | **Network Stack** | ✅ ใช้งานได้ — E1000 + ARP + IPv4 + ICMP + TCP (sliding window + retransmit, Phase 66; single-slot OOO reassembly, Phase 72) + UDP + DHCP + DNS + userland socket fds + TCP server (SYS_LISTEN/ACCEPT, Phase 65) | สูง |
| 17 | **AC97 / Intel HDA Audio** | ❌ ยังไม่ทำ — ไม่มีโค้ด AC97/HDA ใน repo | กลาง |
| 18 | **FAT32 Filesystem** | ✅ Read-only ทำแล้ว (Phase 67) — mount/list/read ผ่าน bcache, 8.3 names; mount เข้า kfs_* VFS tree ที่ /mnt/fat32 แล้ว (Phase 70); ยังไม่ทำ: write, LFN | กลาง |
...
| 21 | **ext2 Filesystem** | ✅ Read-only ทำแล้ว (Phase 68) — superblock/inode/dir walk, direct+single-indirect; mount เข้า kfs_* VFS tree ที่ /mnt/ext2 แล้ว (Phase 70); ยังไม่ทำ: write, double-indirect | กลาง |
...
Phase 28: Shell Pipeline Support ✅ DONE
  ...
  Phase 31: Minimal Shared-Library Loader ✅ DONE
    ...
    Phase 43: fork() + exec() ✅ DONE (baseline)
      ...
      └─ CoW page tables ✅ ทำแล้ว (Phase 69); blocking waitpid ✅ ทำแล้ว (Phase 71)
    ...
    Phase 52: Userspace Socket API ✅ DONE (2026-07-03)
      ...
      └─ ยังไม่ทำ: sendto/recvfrom แยก (SYS_LISTEN/ACCEPT ✅ Phase 65)
...
---
    ...
    Phase 72: TCP Out-of-Order Reassembly ✅ DONE (2026-08-19)
      └─ TcpControlBlock ได้ single-slot OOO buffer: ooo_valid/ooo_seq/ooo_len/
        ooo_buf[TCP_MAX_SEG] — เก็บ segment ที่มาถึงก่อน gap (seq > ack_num)
        แทนที่จะ drop ทิ้งเหมือนเดิม (Phase 66 มีแต่ dup-ACK ไม่เก็บ)
      └─ เมื่อ gap-filler มาถึง (seq == ack_num): store เข้า rx_buf ตามปกติ
        แล้วเช็คทันทีว่า ooo_valid && ooo_seq == ack_num ใหม่ → flush buffered
        segment ต่อเข้า rx_buf, เลื่อน ack_num ผ่านทั้งสอง segment ในทีเดียว
      └─ ผู้ส่งไม่ต้อง retransmit segment ที่มาถึงก่อนแล้ว (เดิม go-back-N ต้อง
        resend ทุก segment ตั้งแต่ตัวที่ acked ล่าสุด แม้ตัวหลังจะถึงจริงแล้ว)
      └─ Host-side test ใหม่: kernel/net/tests/test_tcp_ooo.c — จำลอง handshake
        เต็ม (SYN→SYN-ACK→ACK) ผ่าน g_tcbs โดยตรง แล้วส่ง "WORLD" (seq+5) ก่อน
        "HELLO" (seq base) — ยืนยัน tcp_recv คืน "HELLOWORLD" ครบ 10 byte และ
        ack_num เลื่อนผ่านทั้งคู่ โดยไม่ต้อง resend "WORLD"
          cc -std=c11 -Wall -Iinclude -o /tmp/test_tcp_ooo \
            kernel/net/tcp.c kernel/net/tests/test_tcp_ooo.c && /tmp/test_tcp_ooo
      └─ Scope: single-slot buffer (1 segment ล่วงหน้า) ไม่ใช่ full reassembly
        queue — ครอบคลุม reordering ปกติ (2 segment สลับกัน); segment ที่ 2 มา
        ก่อนขณะมี ooo_valid อยู่แล้วจะถูก drop เหมือนเดิม (ยังไม่ใช่ multi-hole)
      └─ Verified: host test PASS, make test-all 31/31 ทั้ง 2 arch,
        test_cli_permission.py 18/18 (ไม่มี regression)

    Phase 71: Blocking waitpid ✅ DONE (2026-07-04)
      └─ SYS_WAITPID เดิม return -1 ทันทีถ้า child ยังรันอยู่ — caller ทุกตัว
        (init, vsh, forktest) ต้อง spin เรียกซ้ำเองพร้อม yield()
      └─ ตอนนี้ block ในเคอร์เนลจริง: sti (เปิด IF ให้ timer IRQ ยังขับ
        scheduler ต่อ) → loop เช็ค scheduler_get_process_info(pid).state
        ทุกรอบด้วย pause → เจอ Terminated/Zombie ก็ return ทันที → cli
        (idiom เดียวกับ sys_connect/sys_accept ที่มีอยู่แล้ว) — bound 30s
        กันค้างถ้า child ไม่มีวัน exit
      └─ เหตุผลใช้ scheduler_get_process_info แทน get_exit_code อย่างเดียว:
        get_exit_code คืน -1 ทั้ง "ยังรัน", "exit code จริงคือ -1", และ
        "pid ไม่รู้จัก" — แยกไม่ออก จะ block ไม่ได้ถ้าไม่รู้สถานะจริง
      └─ pid ไม่รู้จัก (ไม่ใช่ลูก / reap ไปแล้ว) → return -1 ทันที ไม่รอ 30s
      └─ Verified ทั้ง x86+x64: forktest PASS 2 รอบติด (fork/wait/reap ยัง
        ไม่รั่ว), integration suite 31/31 ทั้งสอง arch, cli_permission 18/18

    Phase 70: VFS Mount Integration + Test Suite Fixes ✅ DONE (2026-07-04)
      └─ FAT32/ext2 mount เข้า kfs_* VFS tree ที่ /mnt/fat32, /mnt/ext2:
        kfs_mount_subpath() route path prefix ตรงไป fat32_*/ext2_* — ไม่ต้อง
        แก้ driver เดิม นอกจาก fat32_stat()/ext2_stat() wrapper เล็กๆ
      └─ ls/cat/write ใช้งานได้ทันทีผ่านเส้นทางปกติ (kfs_find_file/
        kfs_read_file/kfs_list_dir); write/append/mkdir/delete บน /mnt/*
        return -1 (read-only backend)
      └─ Bug ที่เจอระหว่างทำ: mount ต้อง fat32_mount/ext2_mount ผ่าน
        bcache_read() — เดิมเรียกใน kfs_init() ซึ่งรันก่อน bcache_init()
        เสมอ ทำให้ mount เงียบๆ fail ทุกครั้ง (VernisFS เองไม่โดนเพราะอ่าน
        ผ่าน disk backend ตรงๆ ก่อน bcache ครอบ) — ย้ายไป
        kfs_mount_extra_volumes() เรียกหลัง bcache_init() แทน
      └─ แก้ integration suite (test_integration.py, test_cli_permission.py):
        เจอ bug จริงในเคอร์เนล 3 ตัวที่ test เดิมพลาดไปเพราะ harness desync —
        ipc_recv ใช้ owner pid แทน queue id (IPC_ERR_INVAL เสมอ),
        check_access(help) เทียบผิด scale (policy 0/1/2 ไม่ใช่ CLI 0/50/100),
        su ไม่รองรับ root passwordless (prompt password เสมอ, empty=cancel)
      └─ Harness bug: ส่ง "\r\n" ทำให้รัน command เปล่าเพิ่ม 1 ครั้ง prompt
        เหลื่อมทุก command ถัดไป; ps เป็น realtime monitor กิน input จนกว่าจะ
        กด q — ต้อง sync ด้วย prompt-pattern ไม่ใช่ fixed-duration read
      └─ Verified: test_integration.py 31/31 ทั้ง x64+x86 (จากเดิม 22+10/31),
        test_cli_permission.py 18/18 (จากเดิม boot timeout — ขาด -vga none)
      └─ ยังไม่ทำ: write/LFN สำหรับ FAT32, write/double-indirect สำหรับ ext2

    Phase 69: Copy-on-Write Fork ✅ DONE (2026-07-04)
      └─ fork ไม่ deep-copy ทุก page อีกแล้ว: parent+child แชร์ frame แบบ
        read-only + PAGE_COW (software bit 9) + per-frame refcount
      └─ Write แรกจากฝั่งไหนก็ตาม → #PF → paging_handle_cow_fault:
        refcount==1 คืน write permission เฉยๆ, >1 copy page ใหม่
      └─ CR0.WP set ทั้งสอง arch — kernel เขียน user buffer ใน syscall
        ก็ CoW-fault ถูกต้อง (ไม่เขียนทับ shared frame เงียบๆ)
      └─ frame_free/frame_alloc รู้จัก refcount: คืน pool เมื่อ ref สุดท้ายตาย
      └─ Bug ที่เจอ+แก้ (pre-existing): SYS_EXIT เรียก scheduler_terminate_current
        ซึ่ง terminate ตาม round-robin cursor ไม่ใช่ task ที่ exit จริง →
        process ผิดตัวโดน mark Terminated, waitpid ไม่เคยได้ exit code;
        เพิ่ม scheduler_terminate_pid(pid, code) แล้วให้ SYS_EXIT ใช้ pid ตรง
      └─ /bin/forktest{32,64}: parent เขียน pattern → fork → child ทับทั้ง
        global+stack buffer → parent ตรวจว่า memory ตัวเองไม่โดนแตะ
      └─ Verified ทั้ง x86+x64: forktest PASS สองรอบติด (frame reclaim ครบ),
        เห็น [cow] private copy บน serial, boot chain (init fork) ปกติ,
        integration suite เท่า baseline ทั้งสอง arch

    Phase 68: ext2 Read-Only Driver ✅ DONE (2026-07-04)
      └─ kernel/fs/ext2.c: superblock (magic/block size/rev), group descriptor
        table, inode table walk, dir enumeration, file read — ผ่าน bcache
      └─ Block size 1024-4096, rev 0 (inode 128B) + rev 1 (s_inode_size),
        direct 12 + single-indirect blocks, case-sensitive names, read-only
      └─ ai/tools/mkfs_ext2.py: pure-Python ext2 formatter (macOS ไม่มี
        e2fsprogs) — single group, 1KB block; volume 4MB ที่ sector 81920;
        os.img โต 40MB → 44MB
      └─ CLI: ext2mount [sector] / ext2ls <path> / ext2cat <path>
      └─ Verified: host unit test 15/15 (รวม multi-block 3KB file content);
        e2e QEMU ทั้ง x86+x64 (mount/ls/cat/nested/case-sensitive/error +
        FAT32 coexist บน disk เดียวกัน); integration suite เท่า baseline
      └─ ยังไม่ทำ: write, double/triple-indirect, mount เข้า kfs_* VFS tree

    Phase 67: FAT32 Read-Only Driver ✅ DONE (2026-07-04)
      └─ kernel/fs/fat32.c: BPB parse (validate FAT32 จริง: root_ent/fatsz16=0),
        FAT chain walk, dir enumeration, file read — I/O ผ่าน bcache ทั้งหมด
      └─ 8.3 names (case-insensitive), LFN entries ถูก skip, subdirectory walk
        ("/SUBDIR/NESTED.TXT"), read-only
      └─ Volume จริง 36MB (spec-compliant ≥65525 clusters) สร้างด้วย mtools
        (mformat/mcopy/mmd) ที่ sector 8192; os.img โต 4MB → 40MB
      └─ CLI: fatmount [sector] / fatls <path> / fatcat <path>
      └─ Verified: host unit test 15/15 (driver ตัวจริง link กับ mock bcache อ่าน
        image ที่ mtools สร้าง); e2e QEMU ทั้ง x86+x64 (mount/ls/cat/nested/
        lowercase/error paths); integration suite เท่า baseline
      └─ ยังไม่ทำ: write support, LFN, mount เข้า kfs_* VFS tree

    Phase 66: TCP Sliding Window + Data Retransmission ✅ DONE (2026-07-04)
      └─ tx ring 2KB/TCB เก็บ byte range ที่ยังไม่ถูก ack [snd_una, seq_num)
      └─ tcp_send clamp ตาม min(remote_window, ring space) — คืนจำนวน byte ที่
        queue ได้ (0 = window เต็ม, caller ลองใหม่); data ที่ drop ตอน ARP miss
        ไม่หายอีกแล้ว (queue ไว้ retransmit)
      └─ Go-back-N: RTO ~0.5s + exponential backoff (สูงสุด 16x), ยอมแพ้หลัง
        retransmit 8 ครั้ง → CLOSED; ACK ใหม่ reset counter + เลื่อน snd_una
      └─ Advertised window คำนวณจาก rx buffer จริง (TCP_RX_BUF_SIZE - rx_len);
        tcp_recv ส่ง window-update ACK เมื่อ buffer ที่เคยเต็มถูก drain
      └─ ACK processing เพิ่มใน CLOSE_WAIT (peer FIN แล้วแต่เรายังส่งได้)
      └─ ยังไม่ทำ: out-of-order reassembly (OOO → dup-ACK + drop, GBN กู้ให้)
      └─ Verified: host-side unit test 28/28 (mock tick+output บังคับ loss/
        partial ACK/zero window/OOO/max-retry); e2e QEMU ทั้ง x86+x64 ผ่าน;
        integration suite เท่า baseline

    Phase 65: Userland TCP Server (SYS_LISTEN/SYS_ACCEPT) ✅ DONE (2026-07-04)
      └─ tcp.c: LISTEN socket ไม่ถูกกินอีกต่อไป — spawn child TCB ต่อ SYN ที่เข้ามา
        (from_listener/accepted flags), listener รับ connection ต่อได้เรื่อยๆ
      └─ tcp_accept(listen_sock): คืน child ที่ ESTABLISHED/CLOSE_WAIT ที่ยังไม่ถูก accept
      └─ SYS_LISTEN(92) + SYS_ACCEPT(93) ทั้ง x86+x64 — bind เก็บ port อย่างเดียว,
        listen เปิด passive socket, accept รอ bounded ~30s (sti/cli เหมือน connect)
        แล้วคืน fd ใหม่ (read/write/close ได้เลย)
      └─ userlib: listen()/accept() wrappers; /bin/tcpsrv{32,64} echo server port 7777
      └─ CLI: tcpaccept <listen-sock> (kernel-side path)
      └─ Verified ทั้ง x86+x64 ผ่าน QEMU hostfwd: exec tcpsrv → host connect →
        echo กลับถูกต้อง → close → exit; CLI listen/accept + tcpstat แสดง
        LISTEN + ESTABLISHED พร้อมกัน; integration suite เท่า baseline (fail 6 ตัว
        pre-existing: selftest/log/policy/users — ไม่เกี่ยว network)

    Phase 64: Start Bar + App Registry (GUI) ✅ DONE (2026-07-03)
      └─ gui/startmenu.rs: app registry (register_app + FFI gui_register_app(label,
        action)) — "register a program" → ปรากฏใน start menu
      └─ Start button ที่มุมซ้ายของ taskbar → คลิกเปิด/ปิด glass start menu
      └─ รายการ: Terminal (open/focus), Close Terminal, System Info (พิมพ์
        resolution+uptime ลง terminal), Clear Terminal, Restart, Shut Down
      └─ Terminal เปิด/ปิดได้: close ผ่าน X ของหน้าต่าง หรือ menu "Close Terminal";
        reopen ผ่าน menu "Terminal" (terminal_reopen: focus ถ้ามี, สร้างใหม่ถ้าปิด)
      └─ System maintenance: Restart/Shut Down → ACPI (system_restart/shutdown)
      └─ compose: menu toggle บังคับ full compose (panel ลอยเหนือ windows),
        partial path ปิดเมื่อ menu เปิด
      └─ Verified (x64 screendump): menu เปิด, System Info พิมพ์ 1920x1080x32 +
        uptime, close→reopen terminal ครบ cycle; GUI typing/TCP/x86 ปกติ
