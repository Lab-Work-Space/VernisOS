...
int selftest_access_rules(void) {
    ...
    // Test 3: check_access("help") → policy scale is 0/1/2 (root/admin/all),
    // mapped to CLI 0/50/100 by policy_map_privilege(). Expect 2 (all users)
    // or 255 (no rule).
    {
        uint8_t priv = ai_kernel_engine_check_access("help", 4);
        if (priv == 2 || priv == 255) test_ok("check_access(help) → user/no-rule");
        else {
            cli_printf("    expected 2 or 255, got %d\n", (int)priv);
...
int selftest_ipc(void) {
    ...
    // Test 4: Receive the message — ipc_recv takes the queue id from
    // ipc_queue_create, not the owner pid
    IpcMessage msg;
    r = ipc_recv((uint32_t)qid, &msg);
