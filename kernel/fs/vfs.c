...
static const VfsFileEntry KFS_NODE_MNT = {
    .filename = "/mnt",
    .type = VFS_TYPE_DIRECTORY,
};

// ===========================================================================
// Phase 70: FAT32/ext2 mounted under /mnt/fat32, /mnt/ext2. Both drivers are
// read-only and expose a resolve/list/read shape (see fat32.c / ext2.c) that
// maps directly onto KfsOps, so path prefix routing is all that's needed —
// no change to the drivers themselves beyond the *_stat() wrappers below.
// ===========================================================================
#define KFS_MNT_FAT32 "/mnt/fat32"
#define KFS_MNT_EXT2  "/mnt/ext2"

// If path is prefix or prefix+"/...", return the FAT32/ext2-relative
// sub-path ("/" for the mount root itself). NULL if path is outside prefix.
static const char *kfs_mount_subpath(const char *path, const char *prefix) {
    size_t i = 0;
    while (prefix[i]) {
...
static const VfsFileEntry *kfs_find_pseudo(const char *path) {
    ...
    if (kfs_streq(path, "/mnt")) return &KFS_NODE_MNT;
...
// Mounted-volume find/list use one shared scratch entry — a single call
// site consumes the returned pointer before the next lookup runs.
static VfsFileEntry g_kfs_mount_scratch;

static const VfsFileEntry *kfs_find_mounted(const char *path) {
    const char *sp;
    uint32_t size;
    int is_dir;

    sp = kfs_mount_subpath(path, KFS_MNT_FAT32);
    if (sp) {
        if (fat32_stat(sp, &size, &is_dir) < 0) return (const VfsFileEntry *)0;
    } else {
        sp = kfs_mount_subpath(path, KFS_MNT_EXT2);
        if (!sp) return (const VfsFileEntry *)0;
        if (ext2_stat(sp, &size, &is_dir) < 0) return (const VfsFileEntry *)0;
    }

    kfs_copy_str(g_kfs_mount_scratch.filename, VFS_MAX_FILENAME, path);
    g_kfs_mount_scratch.start_sector = 0;
    g_kfs_mount_scratch.size = size;
    g_kfs_mount_scratch.type = is_dir ? VFS_TYPE_DIRECTORY : VFS_TYPE_REGULAR;
    g_kfs_mount_scratch.flags = 0;
    g_kfs_mount_scratch.mode = 0; // permissive (read-only backend, no perm bits)
    g_kfs_mount_scratch.uid = 0;
    g_kfs_mount_scratch.gid = 0;
    return &g_kfs_mount_scratch;
}

typedef struct {
    char (*out)[VFS_MAX_FILENAME];
    int max;
    int count;
    const char *dir_path;
} KfsMountListCtx;

static void kfs_mount_list_cb(const char *name, uint32_t size, int is_dir, void *ud) {
    (void)size; (void)is_dir;
    KfsMountListCtx *ctx = (KfsMountListCtx *)ud;
    if (ctx->count >= ctx->max) return;
    char full[VFS_MAX_FILENAME];
    int pos = kfs_copy_str(full, VFS_MAX_FILENAME, ctx->dir_path);
    pos = kfs_append_str(full, VFS_MAX_FILENAME, pos, "/");
    kfs_append_str(full, VFS_MAX_FILENAME, pos, name);
    kfs_copy_str(ctx->out[ctx->count], VFS_MAX_FILENAME, full);
    ctx->count++;
}

// Returns -2 if dir_path isn't under a mount (caller falls through),
// -1 on a mounted-but-not-found error, else the entry count.
static int kfs_list_mounted(const char *dir_path, char out[][VFS_MAX_FILENAME], int max) {
    const char *sp = kfs_mount_subpath(dir_path, KFS_MNT_FAT32);
    if (sp) {
        KfsMountListCtx ctx = { out, max, 0, dir_path };
        return fat32_list(sp, kfs_mount_list_cb, &ctx) < 0 ? -1 : ctx.count;
    }
    sp = kfs_mount_subpath(dir_path, KFS_MNT_EXT2);
    if (sp) {
        KfsMountListCtx ctx = { out, max, 0, dir_path };
        return ext2_list(sp, kfs_mount_list_cb, &ctx) < 0 ? -1 : ctx.count;
    }
    return -2;
}
...
// FAT32/ext2 read through bcache_read(), so this must run after
// bcache_init() — calling it from kfs_init() (which runs before
// bcache_init() sets up the LRU cache VernisFS itself bypasses) makes
// every bcache_read() fail with "not ready" and both mounts silently no-op.
void kfs_mount_extra_volumes(void) {
    if (fat32_mount(FAT32_DEFAULT_LBA) == 0)
        serial_print("[kfs] FAT32 mounted at /mnt/fat32\n");
    if (ext2_mount(EXT2_DEFAULT_LBA) == 0)
        serial_print("[kfs] ext2 mounted at /mnt/ext2\n");
}
...
const VfsFileEntry *kfs_find_file(const char *path) {
    const VfsFileEntry *mounted = kfs_find_mounted(path);
...
int kfs_read_file(const char *path, uint8_t *buf, size_t max_len) {
    ...
    {
        const char *sp = kfs_mount_subpath(path, KFS_MNT_FAT32);
        if (sp) return fat32_read(sp, buf, (uint32_t)max_len);
        sp = kfs_mount_subpath(path, KFS_MNT_EXT2);
        if (sp) return ext2_read(sp, buf, (uint32_t)max_len);
    }
...
int kfs_write_file(const char *path, const uint8_t *data, size_t len) {
    ...
    if (kfs_mount_subpath(path, KFS_MNT_FAT32) || kfs_mount_subpath(path, KFS_MNT_EXT2))
...
int kfs_list_dir(const char *dir_path, char out[][VFS_MAX_FILENAME], int max) {
    ...
    int mounted = kfs_list_mounted(dir_path, out, max);
    if (mounted != -2) return mounted < 0 ? -1 : mounted;
    ...
    if (kfs_streq(dir_path, "/")) {
        ...
        count = kfs_list_dir_add(out, max, count, "/mnt");
        return count;
    }
    ...
    if (kfs_streq(dir_path, "/mnt")) {
        count = kfs_list_dir_add(out, max, count, "/mnt/fat32");
        count = kfs_list_dir_add(out, max, count, "/mnt/ext2");
        return count;
    }
