...
impl Scheduler {
    ...
    /// Phase 69: terminate an explicit pid. SYS_EXIT must use this — the
    /// scheduler's round-robin cursor (`current_pid`) is not synchronized
    /// with which task slot is actually executing the syscall, so
    /// terminate_current used to mark the wrong process Terminated and
    /// waitpid never saw the real child's exit code.
    pub fn terminate_pid(&mut self, pid: usize, exit_code: i32) {
        if let Some(proc) = self.processes.get_mut(&pid) {
            proc.state = ProcessState::Terminated;
            proc.exit_code = Some(exit_code);
        }
...
#[no_mangle]
pub extern "C" fn scheduler_terminate_pid(sched: *mut Scheduler, pid: usize, exit_code: i32) {
    if sched.is_null() {
        return;
    }
    let sched = unsafe { &mut *sched };
    sched.terminate_pid(pid, exit_code);
}
