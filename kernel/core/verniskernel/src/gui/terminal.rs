...
/// Open the terminal from the start menu: focus it if it already exists,
/// otherwise recreate its window (centered) with a fresh buffer.
pub unsafe fn terminal_reopen() {
    if TERM.initialized && window::wm_window_exists(TERM.window_id) {
        window::wm_focus_window(TERM.window_id);
        return;
    }
    let comp = crate::gui::compositor::compositor_get();
    let content_w = (TERM_COLS as u32) * FONT_WIDTH;
    let content_h = (TERM_ROWS as u32) * FONT_HEIGHT;
    let win_w = content_w + window::BORDER_WIDTH * 2;
    let win_h = content_h + window::TITLE_BAR_HEIGHT + window::BORDER_WIDTH;
    let tx = ((comp.width.saturating_sub(win_w)) / 2) as i32;
    let ty = ((comp.height.saturating_sub(win_h + 32)) / 2) as i32;
    terminal_create(tx, ty);
    terminal_set_color(10, 0);
    terminal_write_string(b"VernisOS GUI Terminal\n");
    terminal_set_color(7, 0);
    terminal_write_string(b"Type 'help' for available commands.\n\n");
    terminal_show_prompt();
}

/// Append a line of text to the terminal (used by the start menu's
/// System Info action).
pub unsafe fn terminal_println(s: &[u8]) {
    if !TERM.initialized || !window::wm_window_exists(TERM.window_id) {
        return;
    }
