...
pub enum ContentStorage {
    Heap(Vec<u8>),
    Fixed { ptr: *mut u8, cap: usize, len: usize },
}
...
impl ContentStorage {
    ...
    pub fn as_ptr(&self) -> *const u8 {
        match self {
            ContentStorage::Heap(v) => v.as_ptr(),
            ContentStorage::Fixed { ptr, .. } => *ptr as *const u8,
        }
    }
    ...
    pub fn as_mut_ptr(&mut self) -> *mut u8 {
        match self {
            ContentStorage::Heap(v) => v.as_mut_ptr(),
            ContentStorage::Fixed { ptr, .. } => *ptr,
        }
    }
    ...
    pub fn len(&self) -> usize {
        match self {
            ContentStorage::Heap(v) => v.len(),
            ContentStorage::Fixed { len, .. } => *len,
        }
    }
    ...
    /// Resize to exactly `new_len` bytes. Heap always succeeds (reallocates,
    /// zero-fills the new tail via `Vec::resize`). Fixed succeeds only if
    /// `new_len` fits the reserved capacity, and zero-fills the whole
    /// `new_len` range every call — cheap (this only runs on maximize/
    /// restore clicks, not per frame), and guarantees stale pixels from a
    /// previous, larger extent never resurface after shrink-then-grow.
    pub unsafe fn try_resize(&mut self, new_len: usize) -> bool {
        match self {
            ContentStorage::Heap(v) => {
                v.resize(new_len, 0);
                true
            }
            ContentStorage::Fixed { ptr, cap, len } => {
                if new_len > *cap {
                    return false;
                }
...
pub struct Window {
    ...
    pub content_buf: ContentStorage,
    pub content_pitch: u32,
    pub visible: bool,
    pub focused: bool,
    pub minimized: bool,
    pub maximized: bool,
    pub restore_rect: (i32, i32, u32, u32), // pre-maximize (x, y, w, h)
    pub hover: Option<TitleButton>,
}
...
pub unsafe fn wm_create_window(title: &[u8], x: i32, y: i32, w: u32, h: u32) -> u32 {
    ...
    let win = Window {
        ...
        width: w,
        height: h,
        content_buf: ContentStorage::Heap(content_buf),
        content_pitch,
        visible: true,
        focused: true,
        minimized: false,
...
/// Does a window with this id currently exist?
pub unsafe fn wm_window_exists(id: u32) -> bool {
    WM.windows.iter().any(|w| w.id == id)
}
