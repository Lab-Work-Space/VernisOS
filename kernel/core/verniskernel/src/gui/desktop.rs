...
pub unsafe fn desktop_draw_taskbar() {
    ...
    // Window entries (the Start button/label is drawn by startmenu::draw)
    let wm = window::wm_get();
    let mut tx = crate::gui::startmenu::START_BTN_W + 8;
    ...
    // Start button + (optionally) the open start menu, on top of everything.
    crate::gui::startmenu::draw(SCREEN_W, SCREEN_H, TASKBAR_HEIGHT);
}
