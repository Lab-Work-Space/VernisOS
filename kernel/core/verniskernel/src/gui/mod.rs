...
pub unsafe extern "C" fn gui_init(screen_w: u32, screen_h: u32) {
    ...
    // Init the start menu / app registry
    startmenu::startmenu_init();
...
pub unsafe extern "C" fn gui_main_loop_tick() {
    ...
    // Start-menu appearance change forces a full compose (the menu panel
    // floats above the windows, so partial paths can't handle it).
    let menu_changed = startmenu::take_dirty();
    ...
    let layout_dirty = had_mouse_button || window::wm_windows_dirty() || menu_changed;
    if menu_changed {
        // Menu opened or closed: the panel floats above the windows, and a
        // close must repaint whatever was under it — so full compose.
        desktop::desktop_draw_background();
        window::wm_compose_all(true);
        desktop::desktop_draw_taskbar();  // draws the menu if open
        window::wm_windows_rendered();
        window::wm_take_damage();          // consumed by the full repaint
        window::wm_take_taskbar_dirty();
    ...
    if layout_dirty {
        ...
        match damage {
            ...
            None => {
                ...
                // recomposed the old cursor's spot, so restore it explicitly
                // before step 5 draws the new one — otherwise the old sprite
                // stays baked into the frame (two cursors visible).
                cursor::cursor_clear_old();
            }
        }
        window::wm_windows_rendered();
        LAST_FULL_COMPOSE_TICK = current_tick;
...
unsafe fn partial_compose_single_window() -> bool {
    if startmenu::is_open() {
        return false; // menu floats above the window — needs a full compose
    }
...
unsafe fn handle_mouse_down(mx: i32, my: i32, button: u8) {
    ...
    let comp = compositor::compositor_get();
    let (sw, sh) = (comp.width, comp.height);
    let tb = desktop::taskbar_height();

    // Start button toggles the start menu
    if startmenu::hit_start_button(mx, my, sh, tb) {
        startmenu::toggle();
        return;
    }
    // If the menu is open, a click either runs an item or dismisses it
    if startmenu::is_open() {
        match startmenu::hit_item(mx, my, sw, sh, tb) {
            Some(0xFF) => { return; }            // inside panel, no item
            Some(action) => { run_start_action(action); return; }
            None => { startmenu::close(); return; } // clicked away
        }
    }
...
/// Execute a start-menu action.
unsafe fn run_start_action(action: u8) {
    match action {
        startmenu::ACT_TERMINAL => terminal::terminal_reopen(),
        startmenu::ACT_RESTART => system_restart(),
        startmenu::ACT_SHUTDOWN => system_shutdown(),
        _ => {}
    }
}
