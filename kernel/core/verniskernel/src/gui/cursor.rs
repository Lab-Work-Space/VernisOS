...
/// Restore whatever is under the cursor's last-drawn position, for callers
/// about to redraw without going through a recompose that already painted
/// over that region (e.g. a click that caused no window/layout damage).
/// Without this, `cursor_draw_fresh` leaves the old sprite baked into the
/// back buffer, and the next present shows two cursors — the stale old one
/// and the freshly drawn one — until a later full compose wipes it.
pub unsafe fn cursor_clear_old() {
    if !CURSOR_HAS_SAVED {
        return;
    }
    let old_rect = (CURSOR_X, CURSOR_Y, CURSOR_W_CLIP, CURSOR_H_CLIP);
    cursor_restore_under();
    compositor::compositor_mark_dirty_rect(old_rect.0, old_rect.1, old_rect.2, old_rect.3);
}
...
pub unsafe fn cursor_draw_fresh(x: i32, y: i32) {
    ...
    // draw_cursor_shape writes pixels directly into the back buffer,
    // bypassing the blit/fill helpers that self-mark compositor dirty.
    // Callers that reach compositor_present() without any other dirty
    // source (e.g. a click with no layout damage) would silently no-op
    // and leave the new cursor position unflushed until a later full
    // compose forced a redraw.
    compositor::compositor_mark_dirty_rect(x, y, CURSOR_W, CURSOR_H);
}
