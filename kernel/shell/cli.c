...
static int cli_cmd_tcphandshake(CliSession *session, const ParsedCommand *cmd) {
    ...
    if (cli_streq(cmd->argv[1], "connect")) {
        ...
        cli_printf("sock %d: listening on port %u (poll with: tcpaccept %d)\n",
                   sock, port, sock);
    } else {
...
// Phase 65: poll a LISTEN socket for an established spawned connection
static int cli_cmd_tcpaccept(CliSession *session, const ParsedCommand *cmd) {
    (void)session;
    if (cmd->argc < 2) {
        cli_printf("Usage: tcpaccept <listen-sock>\n");
        return 1;
    }
    int lsock = simple_atoi(cmd->argv[1]);
    int child = tcp_accept(lsock);
    if (child < 0) {
        cli_printf("tcpaccept: no pending connection\n");
        return 1;
    }
    cli_printf("sock %d: accepted connection (use tcpsend/tcprecv %d)\n",
               child, child);
    return 0;
}

// Phase 67: FAT32 read-only volume at sector 8192 (see Makefile)
static int cli_cmd_fatmount(CliSession *session, const ParsedCommand *cmd) {
    (void)session;
    uint32_t lba = FAT32_DEFAULT_LBA;
    if (cmd->argc >= 2) lba = (uint32_t)simple_atoi(cmd->argv[1]);
    if (fat32_mount(lba) < 0) {
        cli_printf("fatmount: no FAT32 volume at sector %u\n", lba);
        return 1;
    }
    cli_printf("FAT32 volume mounted (sector %u)\n", lba);
    return 0;
}

static void cli_fatls_cb(const char *name, uint32_t size, int is_dir, void *ud) {
    (void)ud;
    if (is_dir) cli_printf("  %s/\n", name);
    else        cli_printf("  %s  %u bytes\n", name, size);
}

static int cli_cmd_fatls(CliSession *session, const ParsedCommand *cmd) {
    (void)session;
    const char *path = (cmd->argc >= 2) ? cmd->argv[1] : "/";
    if (!fat32_mounted()) { cli_printf("fatls: not mounted (run fatmount)\n"); return 1; }
    int n = fat32_list(path, cli_fatls_cb, 0);
    if (n < 0) { cli_printf("fatls: no such directory: %s\n", path); return 1; }
    cli_printf("%d entries\n", n);
    return 0;
}

static int cli_cmd_fatcat(CliSession *session, const ParsedCommand *cmd) {
    (void)session;
    if (cmd->argc < 2) { cli_printf("Usage: fatcat <path>\n"); return 1; }
    if (!fat32_mounted()) { cli_printf("fatcat: not mounted (run fatmount)\n"); return 1; }
    static uint8_t buf[4096];
    int n = fat32_read(cmd->argv[1], buf, sizeof(buf) - 1);
    if (n < 0) { cli_printf("fatcat: no such file: %s\n", cmd->argv[1]); return 1; }
    buf[n] = '\0';
    cli_printf("%s", (const char *)buf);
    if (n > 0 && buf[n - 1] != '\n') cli_printf("\n");
    return 0;
}

// Phase 68: ext2 read-only volume at sector 81920 (see Makefile)
static int cli_cmd_ext2mount(CliSession *session, const ParsedCommand *cmd) {
    (void)session;
    uint32_t lba = EXT2_DEFAULT_LBA;
    if (cmd->argc >= 2) lba = (uint32_t)simple_atoi(cmd->argv[1]);
    if (ext2_mount(lba) < 0) {
        cli_printf("ext2mount: no ext2 volume at sector %u\n", lba);
        return 1;
    }
    cli_printf("ext2 volume mounted (sector %u)\n", lba);
    return 0;
}

static void cli_ext2ls_cb(const char *name, uint32_t size, int is_dir, void *ud) {
    (void)ud;
    if (is_dir) cli_printf("  %s/\n", name);
    else        cli_printf("  %s  %u bytes\n", name, size);
}

static int cli_cmd_ext2ls(CliSession *session, const ParsedCommand *cmd) {
    (void)session;
    const char *path = (cmd->argc >= 2) ? cmd->argv[1] : "/";
    if (!ext2_mounted()) { cli_printf("ext2ls: not mounted (run ext2mount)\n"); return 1; }
    int n = ext2_list(path, cli_ext2ls_cb, 0);
    if (n < 0) { cli_printf("ext2ls: no such directory: %s\n", path); return 1; }
    cli_printf("%d entries\n", n);
    return 0;
}

static int cli_cmd_ext2cat(CliSession *session, const ParsedCommand *cmd) {
    (void)session;
    if (cmd->argc < 2) { cli_printf("Usage: ext2cat <path>\n"); return 1; }
    if (!ext2_mounted()) { cli_printf("ext2cat: not mounted (run ext2mount)\n"); return 1; }
    static uint8_t buf[4096];
    int n = ext2_read(cmd->argv[1], buf, sizeof(buf) - 1);
    if (n < 0) { cli_printf("ext2cat: no such file: %s\n", cmd->argv[1]); return 1; }
    buf[n] = '\0';
    cli_printf("%s", (const char *)buf);
    if (n > 0 && buf[n - 1] != '\n') cli_printf("\n");
    return 0;
...
static int cli_cmd_ver(CliSession *session, const ParsedCommand *cmd);
...
static const CliBuiltinCommand BUILTIN_COMMANDS[] = {
    ...
    { "ver",      "Show VernisOS version",  cli_cmd_ver,      CLI_PRIV_USER },
    ...
    { "tcpaccept",    "Accept TCP connection", cli_cmd_tcpaccept,    CLI_PRIV_USER },
    { "fatmount",     "Mount FAT32 volume",    cli_cmd_fatmount,     CLI_PRIV_USER },
    { "fatls",        "List FAT32 directory",  cli_cmd_fatls,        CLI_PRIV_USER },
    { "fatcat",       "Print FAT32 file",      cli_cmd_fatcat,       CLI_PRIV_USER },
    { "ext2mount",    "Mount ext2 volume",     cli_cmd_ext2mount,    CLI_PRIV_USER },
    { "ext2ls",       "List ext2 directory",   cli_cmd_ext2ls,       CLI_PRIV_USER },
    { "ext2cat",      "Print ext2 file",       cli_cmd_ext2cat,      CLI_PRIV_USER },
...
static int cli_cmd_ver(CliSession *session, const ParsedCommand *cmd) {
    ...
    (void)cmd;
#ifdef __x86_64__
    cli_printf("VernisOS - x86-64 (64-bit) - build dev\n");
#else
    cli_printf("VernisOS - x86 (32-bit) - build dev\n");
#endif
    return CLI_OK;
}
...
static int cli_cmd_su(CliSession *session, const ParsedCommand *cmd) {
    ...
    // Need root password — unless root is passwordless (same rule as login;
    // an empty line from cli_read_password reads as cancel, so a
    // NO_PASSWORD root could otherwise never be su'd to)
    const UserRecord *root_user = userdb_find_user("root");
    if (!root_user || !(root_user->flags & USER_FLAG_ACTIVE)) {
        cli_printf("Authentication failed\n");
        return CLI_OK;
    }
    if (!(root_user->flags & USER_FLAG_NO_PASSWORD)) {
        cli_printf("Password: ");
        char password[64];
        int pw_len = cli_read_password(password, sizeof(password));
        if (pw_len <= 0) {
