...
#define SYS_LISTEN    92    // Phase 65: listen(fd)
#define SYS_ACCEPT    93    // Phase 65: accept(fd) -> new fd
...
extern void scheduler_terminate_pid(void *sched, size_t pid, int32_t exit_code);
extern int32_t scheduler_get_exit_code(const void *sched, size_t pid);

// Mirrors Rust's FfiProcessInfo (scheduler.rs) — only used to tell "still
// running" apart from "exited with code -1"/"pid never existed", which
// scheduler_get_exit_code() alone can't distinguish (all three read as -1).
typedef struct {
    size_t   pid;
    int32_t  state; // FfiProcessState: 5=Terminated, 6=Zombie
    uint8_t  priority;
    int8_t   nice;
    uint64_t cpu_time_secs;
} KWaitProcInfo;
extern bool scheduler_get_process_info(const void *sched, size_t pid, KWaitProcInfo *out_info);
...
#define FD_TYPE_SOCK_TCP   5   // Phase 52
#define FD_TYPE_SOCK_UDP   6
...
typedef struct {
    ...
    char     path[64];
    uint32_t offset;
    uint8_t  pipe_idx;             // pipe index or kernel socket id (SOCK)
    uint32_t sock_peer_ip;        // Phase 52: UDP peer (host order)
...
typedef struct {
    ...
    uint32_t *pd;                  // per-process address space (kernel tasks: kernel_page_dir)
...
static uint32_t *paging_get_kernel_pd_ptr(void);
uint32_t *paging_create_address_space_32(void);
static void paging_free_user_space_32(uint32_t *pd);
static int  paging_clone_user_pages_32(uint32_t *dst_pd, uint32_t *src_pd);
static void frame_free_32(uint32_t frame);
static void task_switch_cr3_32(int idx);
...
static void fd_close_entry_32(FdEntry32 *e) {
    ...
    if (e->type == FD_TYPE_SOCK_TCP && e->pipe_idx != 0xFF)
        tcp_close(e->pipe_idx);
    if (e->type == FD_TYPE_SOCK_UDP && e->pipe_idx != 0xFF)
        udp_close(e->pipe_idx);
    e->type = FD_TYPE_NONE;
}
...
static int32_t sys_read_fd_32(uint32_t fd_num, uint32_t buf_ptr, uint32_t count) {
    ...
    if (e->type == FD_TYPE_SOCK_TCP && e->pipe_idx != 0xFF) {
        int n = tcp_recv(e->pipe_idx, (void *)buf_ptr, (int)count);
        if (n == 0) task_slots[current_task_idx].ticks_remaining = 1;
        return n;
    }
    if (e->type == FD_TYPE_SOCK_UDP && e->pipe_idx != 0xFF) {
        uint32_t sip; uint16_t sport;
        int n = udp_recvfrom(e->pipe_idx, (void *)buf_ptr, (int)count, &sip, &sport);
        if (n == 0) task_slots[current_task_idx].ticks_remaining = 1;
        return n;
    }
    return -1;
...
static int32_t sys_write_fd_32(uint32_t fd_num, uint32_t buf_ptr, uint32_t count) {
    ...
    if (e->type == FD_TYPE_SOCK_TCP && e->pipe_idx != 0xFF)
        return tcp_send(e->pipe_idx, (const void *)buf_ptr, (int)count);
    if (e->type == FD_TYPE_SOCK_UDP && e->pipe_idx != 0xFF)
        return udp_sendto(e->pipe_idx, e->sock_peer_ip, e->sock_peer_port,
                          (const void *)buf_ptr, (int)count);
    return -1;
}

// Phase 52: socket syscalls (x86)
static int32_t sys_socket_32(uint32_t type) {
    if (current_task_idx < 0) return -1;
    FdEntry32 *fdt = task_slots[current_task_idx].fd_table;
    int fd = -1;
    for (int i = 0; i < FD_MAX; i++) if (fdt[i].type == FD_TYPE_NONE) { fd = i; break; }
    if (fd < 0) return -1;
    fdt[fd].sock_peer_ip = 0; fdt[fd].sock_peer_port = 0; fdt[fd].flags = 3;
    if (type == 1) { fdt[fd].type = FD_TYPE_SOCK_TCP; fdt[fd].pipe_idx = 0xFF; }
    else if (type == 2) {
        int sk = udp_bind(0);
        if (sk < 0) return -1;
        fdt[fd].type = FD_TYPE_SOCK_UDP; fdt[fd].pipe_idx = (uint8_t)sk;
    } else return -1;
    return fd;
}

static int32_t sys_connect_32(uint32_t fd, uint32_t ip, uint32_t port) {
    if (current_task_idx < 0 || fd >= FD_MAX) return -1;
    FdEntry32 *e = &task_slots[current_task_idx].fd_table[fd];
    if (e->type == FD_TYPE_SOCK_UDP) {
        e->sock_peer_ip = ip; e->sock_peer_port = (uint16_t)port; return 0;
    }
    if (e->type != FD_TYPE_SOCK_TCP) return -1;
    int sk = tcp_connect(ip, (uint16_t)port);
    if (sk < 0) return -1;
    e->pipe_idx = (uint8_t)sk;
    __asm__ volatile("sti");
    uint32_t start = kernel_tick; int ok = 0;
    while (kernel_tick - start < (uint32_t)TIMER_HZ * 5) {
        TcpState st = g_tcbs[sk].state;
...
static int32_t sys_bind_32(uint32_t fd, uint32_t port) {
    if (current_task_idx < 0 || fd >= FD_MAX) return -1;
    FdEntry32 *e = &task_slots[current_task_idx].fd_table[fd];
    if (e->type == FD_TYPE_SOCK_UDP) {
        if (e->pipe_idx != 0xFF) udp_close(e->pipe_idx);
        int sk = udp_bind((uint16_t)port);
        if (sk < 0) { e->pipe_idx = 0xFF; return -1; }
        e->pipe_idx = (uint8_t)sk; return 0;
    }
    if (e->type == FD_TYPE_SOCK_TCP) {
        if (e->pipe_idx != 0xFF) return -1; // already listening/connected
        e->sock_peer_port = (uint16_t)port; // Phase 65: listen() opens it
        return 0;
    }
    return -1;
}

// Phase 65: listen(fd) — passive open on the port recorded by bind.
static int32_t sys_listen_32(uint32_t fd) {
    if (current_task_idx < 0 || fd >= FD_MAX) return -1;
    FdEntry32 *e = &task_slots[current_task_idx].fd_table[fd];
    if (e->type != FD_TYPE_SOCK_TCP || e->pipe_idx != 0xFF) return -1;
    if (e->sock_peer_port == 0) return -1; // not bound
    int sk = tcp_listen(e->sock_peer_port);
    if (sk < 0) return -1;
    e->pipe_idx = (uint8_t)sk;
    return 0;
}

// Phase 65: accept(fd) — bounded wait for an established connection,
// returns a new fd. Same sti/cli dance as sys_connect_32.
static int32_t sys_accept_32(uint32_t fd) {
    if (current_task_idx < 0 || fd >= FD_MAX) return -1;
    FdEntry32 *fdt = task_slots[current_task_idx].fd_table;
    FdEntry32 *e = &fdt[fd];
    if (e->type != FD_TYPE_SOCK_TCP || e->pipe_idx == 0xFF) return -1;
    int lsock = e->pipe_idx;
    __asm__ volatile("sti");
    uint32_t start = kernel_tick;
    int child = -1;
    while (kernel_tick - start < (uint32_t)TIMER_HZ * 30) {
        child = tcp_accept(lsock);
        if (child >= 0) break;
        __asm__ volatile("pause");
    }
    __asm__ volatile("cli");
    if (child < 0) return -1;
    int nfd = -1;
    for (int i = 0; i < FD_MAX; i++) if (fdt[i].type == FD_TYPE_NONE) { nfd = i; break; }
    if (nfd < 0) { tcp_close(child); return -1; }
    fdt[nfd].type = FD_TYPE_SOCK_TCP;
    fdt[nfd].pipe_idx = (uint8_t)child;
    fdt[nfd].sock_peer_ip = g_tcbs[child].remote_ip;
    fdt[nfd].sock_peer_port = g_tcbs[child].remote_port;
    fdt[nfd].flags = 3;
    return nfd;
}
...
#endif
#ifndef PAGE_COW_32
#define PAGE_COW_32      0x200  // software bit 9: copy-on-write (Phase 69)
#endif
#ifndef PAGE_SIZE
#define PAGE_SIZE 4096
#endif
static uint32_t frame_alloc_32(void);
static int paging_handle_cow_fault_32(uint32_t *pd, uint32_t fault_addr);
...
uint32_t interrupt_dispatch(InterruptFrame32 *frame) {
    ...
    if (n < 32) {
        ...
        // Phase 69: CoW write fault — user code, or kernel code writing a
        // user buffer inside a syscall (CR0.WP=1). error_code bit 1 = write.
        if (n == 14 && (frame->error_code & 0x2) && current_task_idx >= 0) {
            uint32_t cow_addr = read_cr2_32();
            if (paging_handle_cow_fault_32(task_slots[current_task_idx].pd,
                                           cow_addr) == 0) {
                paging_flush_tlb_32();
                serial_print("[cow] private copy va=0x");
                serial_print_hex(cow_addr & ~0xFFFU);
                serial_print("\n");
                return 0; // resume the faulting instruction
            }
        }
        ...
        if ((frame->cs & 0x3) == 0x3) {
            ...
            if (n == 14 && current_task_idx >= 0) {
                ...
                if (vma) {
                    ...
                    if (fr) {
                        ...
                        paging_map_4k_32(task_slots[current_task_idx].pd ? task_slots[current_task_idx].pd : kernel_page_dir, page_va, fr, pg_flags);
                        paging_flush_tlb_32();
            ...
            if (context_switch_enabled && current_task_idx >= 0) {
                ...
                if (next >= 0) {
                    ...
                    serial_print("\n");
                    uint32_t *dead_pd = task_slots[current_task_idx].pd;
                    task_slots[current_task_idx].pd = 0;
                    current_task_idx = next;
                    kernel_tss32.esp0 = (uint32_t)(task_slots[next].stack + TASK_STACK_SIZE);
                    task_switch_cr3_32(next);
                    if (dead_pd && dead_pd != paging_get_kernel_pd_ptr())
                        paging_free_user_space_32(dead_pd);
        ...
        if (context_switch_enabled && current_task_idx >= 0) {
            ...
            if (cur->ticks_remaining == 0) {
                ...
                if (next != current_task_idx && task_slots[next].active) {
                    ...
                    kernel_tss32.esp0 = (uint32_t)(task_slots[next].stack + TASK_STACK_SIZE);
                    task_switch_cr3_32(next);       // per-process paging
    ...
    if (n == 0x80) {   // INT 0x80 syscall
        ...
        if (num == SYS_USER_TEST) {
            ...
            void *sched = get_kernel_scheduler();
            // Phase 69: terminate the exact exiting pid (see x64 note) —
            // terminate_current marked whatever pid the scheduler cursor
            // pointed at, so waitpid never saw the child's exit code.
            if (sched)
                scheduler_terminate_pid(sched, (size_t)exit_pid, (int32_t)a1);
            if (context_switch_enabled && current_task_idx >= 0) {
                task_slots[current_task_idx].active = 0;
                int next = -1;
                for (int i = 0; i < MAX_TASKS; i++) {
            ...
            // Phase 71: waitpid(pid) — blocks (bounded ~30s) until pid
            // exits instead of returning immediately. Same sti/cli dance as
            // sys_accept_32: the int-0x80 gate cleared IF, re-enable it
            // while waiting so the timer IRQ keeps scheduling other tasks.
            void *sched = get_kernel_scheduler();
            ret = -1;
            if (sched) {
                __asm__ volatile("sti");
                ...
                while (kernel_tick - start < (uint32_t)TIMER_HZ * 30) {
                    KWaitProcInfo info;
                    if (!scheduler_get_process_info(sched, (size_t)a1, &info)) {
                        break; // unknown pid — not a child, or already reaped
                    }
                    if (info.state == 5 /* Terminated */ || info.state == 6 /* Zombie */) {
                ...
                if (sig == 9 || sig == 15) {
                    ...
                    for (int ti = 0; ti < MAX_TASKS; ti++) {
                        if (task_slots[ti].active && task_slots[ti].pid == dst_pid
                            && ti != current_task_idx) {
                            uint32_t *victim_pd = task_slots[ti].pd;
                            task_slots[ti].pd = 0;
                            task_slots[ti].active = 0;
                            scheduler_kill_process(sched, dst_pid);
                            if (victim_pd && victim_pd != paging_get_kernel_pd_ptr())
                                paging_free_user_space_32(victim_pd);
                            break;
                        }
                    }
                }
        ...
        } else if (num == SYS_SOCKET) {
            ret = sys_socket_32(a1);
        } else if (num == SYS_CONNECT) {
            ret = sys_connect_32(a1, a2, a3);
        } else if (num == SYS_BIND) {
            ret = sys_bind_32(a1, a2);
        } else if (num == SYS_LISTEN) {
            ret = sys_listen_32(a1);
        } else if (num == SYS_ACCEPT) {
            ret = sys_accept_32(a1);
        ...
        } else if (num == SYS_SETUID || num == SYS_SETGID) {
            // Phase 60: only root may change credentials (no-op self-set ok)
            TaskSlot32 *t = (current_task_idx >= 0) ? &task_slots[current_task_idx] : 0;
            uint16_t v = (uint16_t)a1;
            if (!t) ret = -1;
            else if (num == SYS_SETUID) {
                if (t->uid == 0 || v == t->uid) { t->uid = v; ret = 0; } else ret = -1;
            } else {
                if (t->uid == 0 || v == t->gid) { t->gid = v; ret = 0; } else ret = -1;
            }
        } else if (num == SYS_GETUID) {
            ret = (current_task_idx >= 0) ? (int32_t)task_slots[current_task_idx].uid : 0;
        } else if (num == SYS_GETGID) {
            ret = (current_task_idx >= 0) ? (int32_t)task_slots[current_task_idx].gid : 0;
        } else if (num == SYS_CHDIR) {
            // Phase 60: chdir(path_ptr) — path must exist in the VFS (or be /)
            TaskSlot32 *t = (current_task_idx >= 0) ? &task_slots[current_task_idx] : 0;
            char path[SYS_IO_PATH_MAX];
            if (t && copy_user_path_32(path, a1) == 0 && path[0] == '/' &&
                (path[1] == '\0' || kfs_find_file(path) != 0)) {
                for (int ci = 0; ci < (int)SYS_IO_PATH_MAX; ci++) t->cwd[ci] = path[ci];
                ret = 0;
            } else ret = -1;
        } else if (num == SYS_GETCWD) {
            // Phase 60: getcwd(buf_ptr, size) -> length or -1
            TaskSlot32 *t = (current_task_idx >= 0) ? &task_slots[current_task_idx] : 0;
            uint32_t len = 0;
            if (t) while (len < SYS_IO_PATH_MAX && t->cwd[len]) len++;
            if (t && a2 > len && user_ptr_range_valid_32(a1, len + 1)) {
                for (uint32_t ci = 0; ci <= len; ci++)
                    *(volatile char *)(uintptr_t)(a1 + ci) = t->cwd[ci];
                ret = (int32_t)len;
            } else ret = -1;
        } else if (num == SYS_UMASK) {
            // Phase 60: umask(mask) -> previous mask
            TaskSlot32 *t = (current_task_idx >= 0) ? &task_slots[current_task_idx] : 0;
            if (t) { uint16_t old = t->file_umask; t->file_umask = (uint16_t)(a1 & 0777); ret = (int32_t)old; }
            else ret = -1;
...
static uint32_t phys_pool_base;   // Phase 69: refcount table index base
static uint32_t phys_frames_used;

#define FRAME_FREELIST_MAX 2048
static uint32_t frame_freelist_32[FRAME_FREELIST_MAX];
static int      frame_freelist_count_32;

// Phase 69 (CoW fork): per-frame reference counts for the 2MB pool.
#define FRAME_POOL_MAX_FRAMES_32 512   // 2MB / 4KB
static uint8_t frame_refcount_32[FRAME_POOL_MAX_FRAMES_32];

static int frame_ref_idx_32(uint32_t frame) {
    if (frame < phys_pool_base) return -1;
    uint32_t i = (frame - phys_pool_base) >> 12;
    return i < FRAME_POOL_MAX_FRAMES_32 ? (int)i : -1;
}

static void frame_ref_inc_32(uint32_t frame) {
    int i = frame_ref_idx_32(frame);
    if (i >= 0 && frame_refcount_32[i] < 255) frame_refcount_32[i]++;
}

static uint8_t frame_ref_get_32(uint32_t frame) {
    int i = frame_ref_idx_32(frame);
    return i >= 0 ? frame_refcount_32[i] : 1;
}
...
static void frame_free_32(uint32_t frame) {
    ...
    int i = frame_ref_idx_32(frame);
    if (i >= 0) {
        if (frame_refcount_32[i] > 1) { frame_refcount_32[i]--; return; } // shared
        frame_refcount_32[i] = 0;
    }
    if (frame_freelist_count_32 < FRAME_FREELIST_MAX)
        frame_freelist_32[frame_freelist_count_32++] = frame;
}

static uint32_t frame_alloc_32(void) {
    uint32_t frame;
    if (frame_freelist_count_32 > 0) {
        frame = frame_freelist_32[--frame_freelist_count_32];
    } else {
    ...
    int ri = frame_ref_idx_32(frame);
    if (ri >= 0) frame_refcount_32[ri] = 1;
    // Zero the frame
...
// Create a new address space for a user process: kernel entries (PSE
// identity + framebuffer) shared by value; the user range (PDEs 64..255,
// va 0x10000000-0x40000000) starts empty.
uint32_t *paging_create_address_space_32(void) {
    uint32_t frame = frame_alloc_32();
    if (!frame) return (uint32_t *)0;
    uint32_t *new_pd = (uint32_t *)frame;
    for (int i = 0; i < 1024; i++) {
        uint32_t e = kernel_page_dir[i];
        if (i >= 64 && i < 256 && (e & PAGE_PRESENT_32) && !(e & PAGE_PS_32))
            e = 0;   // never inherit another process's user page tables
        new_pd[i] = e;
    }
    return new_pd;
}

// Free a process address space (user frames + page tables + the dir).
static void paging_free_user_space_32(uint32_t *pd) {
    if (!pd || pd == kernel_page_dir) return;
    for (int i = 64; i < 256; i++) {
        if (!(pd[i] & PAGE_PRESENT_32) || (pd[i] & PAGE_PS_32)) continue;
        uint32_t *pt = (uint32_t *)(pd[i] & 0xFFFFF000);
        for (int j = 0; j < 1024; j++)
            if (pt[j] & PAGE_PRESENT_32) frame_free_32(pt[j] & 0xFFFFF000);
        frame_free_32((uint32_t)pt);
    }
    frame_free_32((uint32_t)pd);
}

// fork (Phase 69): copy-on-write — share frames read-only with PAGE_COW_32
// set and refcounts bumped; first write faults into
// paging_handle_cow_fault_32() which makes a private copy.
static int paging_clone_user_pages_32(uint32_t *dst_pd, uint32_t *src_pd) {
    for (int i = 64; i < 256; i++) {
        if (!(src_pd[i] & PAGE_PRESENT_32) || (src_pd[i] & PAGE_PS_32)) continue;
        uint32_t *pt = (uint32_t *)(src_pd[i] & 0xFFFFF000);
        for (int j = 0; j < 1024; j++) {
            if (!(pt[j] & PAGE_PRESENT_32)) continue;
            if (pt[j] & PAGE_WRITABLE_32)
                pt[j] = (pt[j] & ~(uint32_t)PAGE_WRITABLE_32) | PAGE_COW_32;
            uint32_t frame = pt[j] & 0xFFFFF000;
            uint32_t flags = pt[j] & 0xFFF; // now RO (+COW if was writable)
            uint32_t va = ((uint32_t)i << 22) | ((uint32_t)j << 12);
            paging_map_4k_32(dst_pd, va, frame, flags);
            frame_ref_inc_32(frame);
        }
    }
    // The parent's PTEs just lost write permission — drop stale TLB entries.
    paging_flush_tlb_32();
    return 0;
}

// Walk to the PTE for va without creating intermediate tables.
static uint32_t *paging_get_pte_32(uint32_t *pd, uint32_t va) {
    if (!pd) return (uint32_t *)0;
    uint32_t pde = pd[va >> 22];
    if (!(pde & PAGE_PRESENT_32) || (pde & PAGE_PS_32)) return (uint32_t *)0;
    uint32_t *pt = (uint32_t *)(pde & 0xFFFFF000);
    return &pt[(va >> 12) & 0x3FF];
}

// Phase 69: resolve a write fault on a CoW page. 0 = handled, -1 = not CoW.
static int paging_handle_cow_fault_32(uint32_t *pd, uint32_t fault_addr) {
    uint32_t *pte = paging_get_pte_32(pd ? pd : kernel_page_dir, fault_addr);
    if (!pte || !(*pte & PAGE_PRESENT_32) || !(*pte & PAGE_COW_32)) return -1;
    uint32_t old_frame = *pte & 0xFFFFF000;
    if (frame_ref_get_32(old_frame) == 1) {
        *pte = (*pte | PAGE_WRITABLE_32) & ~(uint32_t)PAGE_COW_32;
        return 0;
    }
    uint32_t nf = frame_alloc_32();
    if (!nf) return -1;
    memcpy((void *)nf, (const void *)old_frame, PAGE_SIZE);
    *pte = nf | (((*pte & 0xFFF) | PAGE_WRITABLE_32) & ~(uint32_t)PAGE_COW_32);
    frame_free_32(old_frame); // drops one reference
    return 0;
}

static uint32_t *paging_get_kernel_pd_ptr(void) { return kernel_page_dir; }

// Load a task's address space if it isn't already active.
static void task_switch_cr3_32(int idx) {
    uint32_t *target = (idx >= 0 && task_slots[idx].pd)
                           ? task_slots[idx].pd : kernel_page_dir;
    uint32_t cur;
    __asm__ volatile("mov %%cr3, %0" : "=r"(cur));
    if (cur != (uint32_t)target)
        __asm__ volatile("mov %0, %%cr3" : : "r"((uint32_t)target) : "memory");
}
...
static void paging_init_32(void) {
    ...
    phys_pool_base  = phys_next_free;  // Phase 69: refcount table origin
    phys_frames_used = 0;
...
static int task_create_32(void (*entry)(void), uint32_t pid, uint16_t ticks) {
    ...
    task_slots[slot].pd = paging_get_kernel_pd_ptr();
...
static void task_register_main_32(uint32_t pid, uint16_t ticks) {
    ...
    task_slots[0].pd              = paging_get_kernel_pd_ptr();
    current_task_idx = 0;
}
...
static int elf_exec_32(const char *path, uint16_t ticks) {
    ...
    task_slots[slot].mmap_next       = 0;
    // Fresh per-process address space
    uint32_t *task_pd = paging_create_address_space_32();
    if (!task_pd) {
    ...
    for (int i = 0; i < ehdr->e_phnum; i++) {
        ...
        for (uint32_t va = vaddr; va < vend; va += PAGE_SIZE) {
            ...
            paging_map_4k_32(task_pd, va, frame,
    ...
    for (uint32_t i = 0; i < USER_STACK_SIZE_32; i += PAGE_SIZE) {
        ...
        paging_map_4k_32(task_pd, USER_STACK_TOP_32 - USER_STACK_SIZE_32 + i,
                         frame, PAGE_USER_32 | PAGE_WRITABLE_32);
    }
    task_slots[slot].pd = task_pd;
...
static int32_t sys_fork_32(InterruptFrame32 *frame) {
    ...
    // Per-process paging: child gets its own space with a deep copy of the
    // parent's user pages
    task_slots[child_slot].pd = paging_create_address_space_32();
    if (!task_slots[child_slot].pd ||
        paging_clone_user_pages_32(task_slots[child_slot].pd, task_slots[current_task_idx].pd) < 0) {
        if (task_slots[child_slot].pd) paging_free_user_space_32(task_slots[child_slot].pd);
        task_slots[child_slot].active = 0;
        return -1;
    }

    // Phase 60: inherit credentials, umask and cwd
    task_slots[child_slot].uid = task_slots[current_task_idx].uid;
    task_slots[child_slot].gid = task_slots[current_task_idx].gid;
    task_slots[child_slot].file_umask = task_slots[current_task_idx].file_umask;
    for (int ci = 0; ci < (int)SYS_IO_PATH_MAX; ci++)
        task_slots[child_slot].cwd[ci] = task_slots[current_task_idx].cwd[ci];
...
static int32_t sys_execve_32(InterruptFrame32 *frame, uint32_t path_ptr) {
    ...
    for (int i = 0; i < ehdr->e_phnum; i++) {
        ...
        for (uint32_t va = vaddr; va < vend; va += PAGE_SIZE) {
            ...
            paging_map_4k_32(task_pd, va, f, PAGE_USER_32 | PAGE_WRITABLE_32);
        }
    }

    // Fresh user stack in the new space
    for (uint32_t i = 0; i < USER_STACK_SIZE_32; i += PAGE_SIZE) {
        uint32_t f = frame_alloc_32();
        if (!f) return -1;
        paging_map_4k_32(task_pd, USER_STACK_TOP_32 - USER_STACK_SIZE_32 + i,
                         f, PAGE_USER_32 | PAGE_WRITABLE_32);
    }
    // Switch to the new space, then free the old (never free what CR3 points at)
    {
        uint32_t *old_pd = task_slots[current_task_idx].pd;
        task_slots[current_task_idx].pd = task_pd;
        task_switch_cr3_32(current_task_idx);
        if (old_pd && old_pd != paging_get_kernel_pd_ptr())
            paging_free_user_space_32(old_pd);
    }
...
void kernel_main(void) {
    ...
    bcache_init();
    // Phase 70: FAT32/ext2 test volumes under /mnt (need bcache up first)
    kfs_mount_extra_volumes();
    userdb_init();
    // Phase 14: Audit log
    auditlog_init();
    ...
    g_elf_exec_fn = kernel_elf_exec_32;
    {
        ...
        const char *user_boot = vfs_find_file("/sbin/init32") ? "/sbin/init32"
                                                              : "/bin/vsh32";
        ...
        if (vsh) {
            ...
            int elf_slot = elf_exec_32(user_boot, 4);
            if (elf_slot >= 0) {
                if (user_boot[1] == 'b')  // fallback direct-vsh only
