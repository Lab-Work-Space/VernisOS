...
#define SYS_LISTEN    92    // Phase 65: listen(fd) — TCP passive open on bound port
#define SYS_ACCEPT    93    // Phase 65: accept(fd) -> new connection fd
...
// Phase 52: bind(fd, port) — UDP rebinds to a fixed port; TCP records the
// port for a later SYS_LISTEN (Phase 65).
static int64_t sys_bind(FdEntry *fdt, int fd, uint16_t port) {
    ...
    if (fdt[fd].type == FD_TYPE_SOCK_TCP) {
        if (fdt[fd].pipe_idx != 0xFF) return -1; // already listening/connected
        fdt[fd].sock_peer_port = port;
        return 0;
    }
    return -1;
...
static int64_t sys_listen(FdEntry *fdt, int fd) {
    if (fd < 0 || fd >= FD_MAX || fdt[fd].type != FD_TYPE_SOCK_TCP) return -1;
    if (fdt[fd].pipe_idx != 0xFF) return -1;   // already listening/connected
    if (fdt[fd].sock_peer_port == 0) return -1; // not bound
    int sock = tcp_listen(fdt[fd].sock_peer_port);
    if (sock < 0) return -1;
    fdt[fd].pipe_idx = (uint8_t)sock;
    return 0;
}

// Phase 65: accept(fd) — wait (bounded) for an established connection on
// the listening socket, return a new fd for it. Same interrupt dance as
// sys_connect: the int-0x80 gate cleared IF, re-enable so the timer IRQ
// keeps driving the rx poll + tcp_tick while we wait.
static int64_t sys_accept(FdEntry *fdt, int fd) {
    if (fd < 0 || fd >= FD_MAX || fdt[fd].type != FD_TYPE_SOCK_TCP) return -1;
    if (fdt[fd].pipe_idx == 0xFF) return -1;   // not listening
    int lsock = fdt[fd].pipe_idx;
    __asm__ volatile("sti");
    uint64_t start = kernel_tick;
    int child = -1;
    while (kernel_tick - start < (uint64_t)TIMER_HZ * 30) {
...
#endif
#ifndef PAGE_COW
#define PAGE_COW        0x200ULL  // software bit 9: copy-on-write (Phase 69)
#endif
static uint64_t frame_alloc(void);
static int paging_handle_cow_fault(uint64_t *pml4, uint64_t fault_addr);
static void paging_map_4k(uint64_t *pml4, uint64_t virt, uint64_t phys, uint64_t flags);
...
uint64_t interrupt_dispatch(InterruptFrame *frame) {
    ...
    if (vec == 0x80) {
        ...
        if (frame->rax == SYS_USER_TEST) {
            ...
            void *sched = get_kernel_scheduler();
            // Phase 69: terminate the exact exiting pid — the scheduler's
            // round-robin cursor is not the task that made this syscall,
            // so terminate_current marked the wrong process and waitpid
            // never saw the child's exit code.
            if (sched)
                scheduler_terminate_pid(sched, (size_t)exit_pid, (int32_t)frame->rbx);
            ...
            // exits instead of returning immediately; callers that used to
            // spin-poll with yield() now block for real. Same sti/cli dance
            // as sys_connect/sys_accept: the int-0x80 gate cleared IF, so
            // re-enable it while waiting so the timer IRQ keeps scheduling
            // other tasks (including the child we're waiting on).
            void *sched = get_kernel_scheduler();
            int32_t code = -1;
            if (sched) {
                __asm__ volatile("sti");
                ...
                while (kernel_tick - start < (uint64_t)TIMER_HZ * 30) {
                    KWaitProcInfo info;
                    if (!scheduler_get_process_info(sched, (size_t)frame->rbx, &info)) {
                        break; // unknown pid — not a child, or already reaped
                    }
                    if (info.state == 5 /* Terminated */ || info.state == 6 /* Zombie */) {
                        code = scheduler_get_exit_code(sched, (size_t)frame->rbx);
                        break;
                    }
                    __asm__ volatile("pause");
                }
        ...
        } else if (frame->rax == SYS_LISTEN) {
            // Phase 65: listen(fd)
            FdEntry *fdt = (current_task_idx >= 0) ? task_slots[current_task_idx].fd_table : (FdEntry*)0;
            frame->rax = fdt ? (uint64_t)sys_listen(fdt, (int)frame->rbx) : (uint64_t)-1;
        } else if (frame->rax == SYS_ACCEPT) {
            // Phase 65: accept(fd) -> connection fd
            FdEntry *fdt = (current_task_idx >= 0) ? task_slots[current_task_idx].fd_table : (FdEntry*)0;
            frame->rax = fdt ? (uint64_t)sys_accept(fdt, (int)frame->rbx) : (uint64_t)-1;
    ...
    if (vec <= 19) {
        ...
        if (vec == 14 && (frame->error_code & 0x2) && current_task_idx >= 0) {
            uint64_t cow_addr = read_cr2();
            if (paging_handle_cow_fault(task_slots[current_task_idx].pml4,
                                        cow_addr) == 0) {
                paging_flush_tlb();
                serial_print("[cow] private copy va=0x");
                serial_print_hex(cow_addr & ~0xFFFULL);
                serial_print("\n");
...
// NOTE: do not enlarge — BSS end + the 4MB frame-allocator pool must stay
...
static uint64_t phys_pool_base;   // Phase 69: refcount table index base
...
// Phase 69 (CoW fork): per-frame reference counts for the 4MB pool.
// A frame is returned to the free list only when its last reference dies.
#define FRAME_POOL_MAX_FRAMES 1024   // 4MB / 4KB
static uint8_t frame_refcount[FRAME_POOL_MAX_FRAMES];

static int frame_ref_idx(uint64_t frame) {
    if (frame < phys_pool_base) return -1;
    uint64_t i = (frame - phys_pool_base) >> 12;
    return i < FRAME_POOL_MAX_FRAMES ? (int)i : -1;
}

static void frame_ref_inc(uint64_t frame) {
    int i = frame_ref_idx(frame);
    if (i >= 0 && frame_refcount[i] < 255) frame_refcount[i]++;
}

static uint8_t frame_ref_get(uint64_t frame) {
    int i = frame_ref_idx(frame);
    return i >= 0 ? frame_refcount[i] : 1;
}
...
static void frame_free(uint64_t frame) {
    ...
    int i = frame_ref_idx(frame);
    if (i >= 0) {
        if (frame_refcount[i] > 1) { frame_refcount[i]--; return; } // shared
        frame_refcount[i] = 0;
    }
...
static uint64_t frame_alloc(void) {
    ...
    int ri = frame_ref_idx(frame);
    if (ri >= 0) frame_refcount[ri] = 1;
...
// fork (Phase 69): copy-on-write. Instead of deep-copying every user page,
// both parent and child map the same frame read-only with PAGE_COW set and
// the frame's refcount bumped; the first write from either side takes a #PF
// that paging_handle_cow_fault() resolves with a private copy.
static int paging_clone_user_pages(uint64_t *dst_pml4, uint64_t *src_pml4) {
    ...
    for (int i = 64; i < 512; i++) {
        ...
        for (int j = 0; j < 512; j++) {
            ...
            if (pt[j] & PAGE_WRITABLE)
                pt[j] = (pt[j] & ~PAGE_WRITABLE) | PAGE_COW;
            uint64_t frame = pt[j] & PAGE_ADDR_MASK;
            uint64_t flags = pt[j] & 0xFFFULL; // now RO (+COW if was writable)
            uint64_t va = ((uint64_t)i << 21) | ((uint64_t)j << 12);
            paging_map_4k(dst_pml4, va, frame, flags);
            frame_ref_inc(frame);
        }
    }
...
static uint64_t *paging_get_pte(uint64_t *pml4, uint64_t va) {
    if (!pml4) return (uint64_t *)0;
    uint64_t *pdpt, *pd, *pt;
    if (!(pml4[(va >> 39) & 0x1FF] & PAGE_PRESENT)) return (uint64_t *)0;
    pdpt = (uint64_t *)(pml4[(va >> 39) & 0x1FF] & PAGE_ADDR_MASK);
    if (!(pdpt[(va >> 30) & 0x1FF] & PAGE_PRESENT) ||
        (pdpt[(va >> 30) & 0x1FF] & PAGE_PS)) return (uint64_t *)0;
    pd = (uint64_t *)(pdpt[(va >> 30) & 0x1FF] & PAGE_ADDR_MASK);
    if (!(pd[(va >> 21) & 0x1FF] & PAGE_PRESENT) ||
        (pd[(va >> 21) & 0x1FF] & PAGE_PS)) return (uint64_t *)0;
    pt = (uint64_t *)(pd[(va >> 21) & 0x1FF] & PAGE_ADDR_MASK);
    return &pt[(va >> 12) & 0x1FF];
}

// Phase 69: resolve a write fault on a CoW page. Returns 0 if handled
// (caller flushes the TLB and resumes), -1 if this isn't a CoW fault.
static int paging_handle_cow_fault(uint64_t *pml4, uint64_t fault_addr) {
    uint64_t *pte = paging_get_pte(pml4 ? pml4 : kernel_pml4, fault_addr);
    if (!pte || !(*pte & PAGE_PRESENT) || !(*pte & PAGE_COW)) return -1;
    uint64_t old_frame = *pte & PAGE_ADDR_MASK;
    if (frame_ref_get(old_frame) == 1) {
        // Sole owner left — just restore write permission.
        *pte = (*pte | PAGE_WRITABLE) & ~PAGE_COW;
        return 0;
    }
    uint64_t nf = frame_alloc();
    if (!nf) return -1; // pool exhausted: fall through to the kill path
    memcpy((void *)nf, (const void *)old_frame, PAGE_SIZE);
    *pte = nf | (((*pte & 0xFFFULL) | PAGE_WRITABLE) & ~PAGE_COW);
    frame_free(old_frame); // drops one reference
    return 0;
}
...
static void paging_init(void) {
    ...
    // Phase 69: CR0.WP so kernel-mode writes honor read-only PTEs — without
    // it a syscall writing into a user buffer would silently modify a shared
    // CoW frame instead of faulting into paging_handle_cow_fault().
    {
        uint64_t cr0;
