...
class QemuSession:
    ...
    def start(self):
        ...
        cmd = [
            ...
            "-serial", "stdio",
            "-display", "none",
            # Force VGA-text CLI mode: with a VGA device present the kernel
            # boots into the GUI and the serial shell never prompts.
            "-vga", "none",
            "-no-reboot",
            "-no-shutdown",
        ]
...
def test_root_commands(session, results):
    ...
    # ps (USER level) — realtime monitor: swallows input until 'q'/Esc,
    # so check the first table render then exit with a bare 'q'
    session.send("ps")
    output = session.read_until("COMMAND", timeout=5)
    if "COMMAND" in output or "PID" in output:
        results.ok("root can run 'ps'")
    else:
        results.fail("root 'ps' failed", output.strip()[:80])
    session.send_raw("q")
    session.read_until("@vernisOS", timeout=5)
...
def test_login_admin(session, results):
    ...
    time.sleep(0.5)
    # Should prompt for password ("Password: " — don't match ':' alone,
    # leftover prompt tails like ':/> ' contain one too)
    output = session.read_until("Password", timeout=3)
...
def test_su_command(session, results):
    ...
    session.send("login user")
    time.sleep(0.5)
    session.read_until("Password", timeout=3)
    session.send("user")
    time.sleep(0.5)
    session.read_until("@vernisOS", timeout=3)

    # su with a command — root is seeded passwordless, so su runs the
    # command directly with no password prompt. If root has a password,
    # a "Password:" prompt appears and we can't authenticate here.
    session.send("su whoami")
    time.sleep(0.5)
    output = session.read_until("@vernisOS", timeout=3)
    if "Password" in output:
        results.skip("su whoami (root has a password)", output.strip()[:80])
        session.send("")  # cancel the prompt
        session.read_until("@vernisOS", timeout=3)
    elif "root" in output:
        results.ok("su whoami shows root (passwordless root)")
    else:
        results.fail("su whoami result unclear", output.strip()[:80])
