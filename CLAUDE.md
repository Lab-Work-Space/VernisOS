...
- GUI perf model (QEMU is TCG on Apple Silicon — no hardware accel for x86 guests, so per-pixel work is ~100x native): typing restores the cached glass base with a memcpy (zero blur per keystroke); dragging renders tint-only glass at ~60fps and re-frosts on drop; full composes log `[gui] slow compose: ticks=N` to serial when they exceed ~33ms. Avoid per-pixel divides and per-pixel helper calls in render loops — use the 32bpp fast paths as templates
