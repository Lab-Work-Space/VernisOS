...
Each user process has its own address space (`TaskSlot.pml4` on x64, `.pd` on x86). `paging_create_address_space` clones the 0–1GB paging chain — kernel identity + framebuffer mappings are shared by value, the user range starts empty. CR3 is switched on context switch (timer), execve, exit and kill. `elf_exec`/`execve` build into a fresh space then free the old one *after* switching CR3 (never free what CR3 still points at); `fork` deep-copies every mapped user page. Frames are reclaimed via `frame_free` into a free-list, so exec/exit/kill cycles don't exhaust the pool. `/sbin/init{32,64}` (userlib/init.c) boots as the first user process.

Multiuser login chain (Phase 60): init execs `/sbin/getty{32,64}` (banner) → `/bin/login{32,64}` (authenticate against `/etc/shadow` via `SYS_AUTH(90)`, which runs the kernel userdb + SHA-256, returning a uid) → `setuid`/`setgid` → create+enter home (`/root` or `/home/<user>`, via `SYS_MKDIR2(91)` + `chdir`) → exec `/bin/vsh{32,64}`. Seeded users: root (no password), admin/admin, user/user. Console ownership: the kernel CLI (cli.c) owns the serial line by default (serial → keyboard buffer); the `console` CLI command flips `g_tty_console` so serial routes to the userland TTY instead, handing the terminal to the getty/login/vsh chain (and cli_readline idles while handed off). A kernel "kinit" task is the respawn fallback if init is absent.
